...
const Todo = () => {
  ...
  const [filter, setFilter] = useState<'completed' | 'pending' | 'todo' | 'all'>('all');
    ...
    >
          ...
          <div className="flex gap-4 h-full flex-wrap sm:flex-nowrap items-center justify-between sz:justify-normal">
            <div className="flex flex-col sz:h-full gap-3 justify-between">
              ...
              >
                ...
                <div
                  data-testid="allTodoNumber"
                  className="w-6 h-6 flex items-center justify-center bg-background dark:bg-background-dark rounded-[8px]"
                >
                  {all}
                </div>
              </div>
      ...
      {filteredTodos.length === 0 && !isLoading && (
        <p className="text-center text-lg font-semibold">
          {filter === 'all' ? 'You have no todos !!' : `You have no ${filter} todos !!`}
        </p>
      )}
      {isLoading && (
